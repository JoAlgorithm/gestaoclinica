...
export class ConsultasComponent implements OnInit, AfterViewInit {
  ...
  displayedColumnsPendentes = ['data','tipo','nid','apelido', 'nome' , 'cancelar', 'atender'];
  ...
  applyFilter2(filterValue: string) {
    filterValue = filterValue.trim(); // Remove whitespace
    filterValue = filterValue.toLowerCase(); // Datasource defaults to lowercase matches
    this.dataSourseAndamento.filter = filterValue;
  }

  applyFilter3(filterValue: string) {
    filterValue = filterValue.trim(); // Remove whitespace
    filterValue = filterValue.toLowerCase(); // Datasource defaults to lowercase matches
    this.dataSourseEncerradas.filter = filterValue;
  }
